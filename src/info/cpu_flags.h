...
struct cpu_flags_names
{
    char* abbreviated_name;
    char* full_name;
};
// ПОМЕНЯТЬ ПОЛНЫЕ ИМЕНА У ОСНОВНЫХ ФЛАГОВ
const struct cpu_flags_names flag_full_name_storage[] = 
{
...
/* bug workarounds */
    ...
    { "bug:cpu_insecure",      "CPU is affected by meltdown attack and needs kernel page table isolation" },
    { "bug:cpu_meltdown",       "CPU is affected by meltdown attack and needs kernel page table isolation" },
    { "bug:spectre_v1",      "CPU is affected by Spectre variant 1 attack with conditional branches" },
    { "bug:spectre_v2",       "CPU is affected by Spectre variant 2 attack with indirect branches" },
    { "bug:spec_store_bypass",  "CPU is affected by speculative store bypass attack" },
    { "bug:l1tf",   "CPU is affected by L1 Terminal Fault" },
/* power management
