...
enum reporting_page
{
    REPORT_CPU_INFO       = 1,
    REPORT_CPU_LOAD       = 2,
    REPORT_CPU_CORES_LOAD = 4,
    REPORT_MEMORY         = 5,
    REPORT_MEMORY_LOAD    = 6,
    REPORT_NETWORK_STATS  = 7,
    REPORT_PCI_INFO       = 9,
    REPORT_SENSORS        = 10
};

void make_system_report(system_t* data, const int ref_time, const int page)
{
    ...
    switch (page)
    {
    case REPORT_CPU_INFO:
    {
        fprintf(file_ptr, "Information collected: CPU\n\n");
    ...
    case REPORT_CPU_LOAD:
    {
        fprintf(file_ptr, "Information collected: CPU Total Usage\n\n");

        fprintf(file_ptr, " - Current CPU Total Usage: %0.1f%%", data->cpu->current_load.load_history[data->cpu->current_load.cur_point - 1].user 
                                                                + data->cpu->current_load.load_history[data->cpu->current_load.cur_point - 1].sys 
                                                                + data->cpu->current_load.load_history[data->cpu->current_load.cur_point - 1].wait);

        break;
    }
    case REPORT_CPU_CORES_LOAD:
    {
        fprintf(file_ptr, "Information collected: CPU Each Usage\n\n");
    ...
    case REPORT_MEMORY:
    {
        fprintf(file_ptr, "Information collected: Memory\n\n");
        ...
        fprintf(file_ptr, " - Swap Free:   %0.1fMiB(%3.1lf%%)", data->memory->usage_stats.swap_free / KiB,
                                                                data->memory->usage_percentage.swap_free);

        break;
    }
    case REPORT_MEMORY_LOAD:
    {
        fprintf(file_ptr, "Information collected: Memory Usage\n\n");

        fprintf(file_ptr, " - RAM Usage: %3.1lf%%\n\n", data->memory->usage_percentage.usage);
        fprintf(file_ptr, " - Swap Usage: %3.1lf%%", data->memory->usage_percentage.swap_usage);

        break;
    }
    case REPORT_NETWORK_STATS:
    {
        fprintf(file_ptr, "Information collected: Network Bandwidth\n\n");
    ...
    case REPORT_PCI_INFO:
    {
        fprintf(file_ptr, "Information collected: PCI Devices\n\n");
    ...
    case REPORT_SENSORS:
    {
        fprintf(file_ptr, "Information collected: Sensors\n\n");
