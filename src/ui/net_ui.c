...
void print_network_bandwitdh_graph(WINDOW* main_page, net_t* network, const int time, const int intf_id, const int graph_width)
{
	...
	wattrset(main_page, COLOR_PAIR(PAIR_BLUE_WHITE));
    mvwprintw_clr(main_page, 3, 0, "Curr: %0.2f KB/time", network->stat[intf_id].r_bandwith.cur / KB);
    mvwprintw_clr(main_page, 4, 0, "Avg:  %0.2f KB/time", network->stat[intf_id].r_bandwith.avg / KB);
    mvwprintw_clr(main_page, 5, 0, "Min:  %0.2f KB/time", network->stat[intf_id].r_bandwith.min / KB);
    mvwprintw_clr(main_page, 6, 0, "Max:  %0.2f KB/time", network->stat[intf_id].r_bandwith.max / KB);
    mvwprintw_clr(main_page, 7, 0, "Ttl:  %0.2f MB", network->stat[intf_id].r_bytes / MB);

	wattrset(main_page, COLOR_PAIR(PAIR_WHITE_RED));
    mvwprintw(main_page, 2, 25, "Transmitted:");

	wattrset(main_page, COLOR_PAIR(PAIR_BLUE_WHITE));
    mvwprintw_clr(main_page, 3, 25, "Curr: %0.2f KB/time", network->stat[intf_id].t_bandwith.cur / KB);
    mvwprintw_clr(main_page, 4, 25, "Avg:  %0.2f KB/time", network->stat[intf_id].t_bandwith.avg / KB);
    mvwprintw_clr(main_page, 5, 25, "Min:  %0.2f KB/time", network->stat[intf_id].t_bandwith.min / KB);
    mvwprintw_clr(main_page, 6, 25, "Max:  %0.2f KB/time", network->stat[intf_id].t_bandwith.max / KB);
    mvwprintw_clr(main_page, 7, 25, "Ttl:  %0.2f MB", network->stat[intf_id].t_bytes / MB);
